...
import type { WorldTodayCard } from "@/lib/worldToday";
...
async function loadFirebaseServices(): Promise<FirebaseServices | null> {
  ...
  firebaseServicesPromise = Promise.all([
    ...
    import("@/lib/appCheck"),
  ]).then(([firebaseApp, firebaseAuth, firebaseFirestore, firebaseFunctions, appCheck]) => {
    const app = firebaseApp.getApps()[0] ?? firebaseApp.initializeApp(firebaseConfig);
    appCheck.activateClientAppCheck(app);
    return {
      app,
      auth: firebaseAuth.getAuth(app),
      firestore: firebaseFirestore.getFirestore(app),
      functions: firebaseFunctions.getFunctions(app, "us-central1"),
    };
  });
...
export default function LandingPage({ initialWorld }: { initialWorld: WorldTodayCard }) {
  ...
  const [liveCount, setLiveCount] = useState(initialWorld.answerCount);
  const [worldMembers, setWorldMembers] = useState(initialWorld.memberCount);
  const [worldGoal, setWorldGoal] = useState(initialWorld.worldGoal);
  const [user, setUser] = useState<User | null>(null);
  const [services, setServices] = useState<FirebaseServices | null>(null);
  const playStartedRef = useRef(false);

  const ensureFirebaseServices = useCallback(async () => {
    if (services) return services;
    const loaded = await loadFirebaseServices();
  ...
  useEffect(() => {
    ...
    void import("firebase/firestore").then(({ doc, onSnapshot }) => {
      if (cancelled) return;
      unsubscribe = onSnapshot(doc(services.firestore, "rooms", "world"), (snapshot) => {
  ...
  useEffect(() => {
    if (!services || !worldDailyKey) return undefined;
    let unsubscribe: (() => void) | undefined;
    let cancelled = false;
    void import("firebase/firestore").then(({ doc, onSnapshot }) => {
      if (cancelled) return;
      unsubscribe = onSnapshot(
        doc(services.firestore, "rooms", "world", "days", worldDailyKey),
        (snapshot) => {
          const data = snapshot.data();
          ...
          // Never swap the question deck underneath someone who has already
          // chosen a side (for example, during a midnight rollover).
          if (!playStartedRef.current) setWorldQuestions(nextQuestions);
  ...
  function pick(side: "a" | "b") {
    ...
    if (!playStartedRef.current) {
      ...
      // Re-assert the deck from this rendered click so a just-arrived live
      // snapshot cannot replace question 1 while the event is being handled.
      setWorldQuestions(worldQuestions);
    }
  ...
  async function signInOrCreateLandingUser(normalizedEmail: string, nextPassword: string) {
    ...
    if (!activeServices) throw new Error("Live authentication is unavailable.");
    ...
    try {
      ...
      if (!["auth/invalid-credential", "auth/user-not-found", "auth/wrong-password"].includes(signInCode)) {
        throw signInError;
      }
      try {
        ...
        if (firebaseErrorCode(createError) === "auth/email-already-in-use") {
  ...
  async function createAppHandoff(targetRoute: string) {
    const activeServices = await ensureFirebaseServices();
    if (!activeServices) return `${appBaseUrl}${targetRoute}`;
    const { httpsCallable } = await import("firebase/functions");
    const callable = httpsCallable(activeServices.functions, "createAuthHandoff");
  ...
  async function completeLandingPlay() {
    ...
    try {
      ...
      if (activeServices) {
        try {
          const { httpsCallable } = await import("firebase/functions");
          await httpsCallable(activeServices.functions, "lockRoomAnswers")({ roomId: "world", picks });
        } catch (lockError) {
          const code = firebaseErrorCode(lockError);
      ...
      setSubmitError(firebaseErrorCode(error) ? authErrorMessage(error) : errorMessage(error));
      setSubmitting("");
    }
  }
